...
def processa_video_grande(file_path):
    ...
    for subclip in subclip_files:
        ...
        if not video_caricato(subclip, LOG_FILE_SUBCLIPS):
            ...
            if upload_single.upload_single_video(API_ID, API_HASH, BOT_TOKEN, CHANNEL_USERNAME, subclip, thumbnail_path):
...
def processa_video_minore(file_path):
    ...
    if upload_single.upload_single_video(API_ID, API_HASH, BOT_TOKEN, CHANNEL_USERNAME, file_path, thumbnail_path):
        aggiungi_a_log(file_path, LOG_FILE_VIDEOS)
